-- Corrected Credit Account System
-- Customers have money in their account (positive balance = money they have)
-- When they buy on credit, it subtracts from their balance
-- If they don't have enough, it goes negative (they owe money)
...
CREATE TABLE customers (
    ...
    account_balance NUMERIC(10, 2) DEFAULT 0, -- Money they have in their account
    credit_limit NUMERIC(10, 2) DEFAULT 1000, -- Maximum they can go into debt
...
CREATE TABLE credit_transactions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    customer_id UUID NOT NULL REFERENCES customers(id) ON DELETE CASCADE,
    type VARCHAR(20) NOT NULL CHECK (type IN ('purchase', 'payment', 'refund', 'adjustment', 'deposit')),
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        INSERT INTO credit_accounts (customer_id, account_balance, credit_limit, is_active)
        VALUES (NEW.id, NEW.account_balance, NEW.credit_limit, NEW.status = 'active')
...
-- Function to process credit purchase (subtract from account balance)
...
BEGIN
    ...
    -- Calculate available credit (how much they can go into debt)
    v_available_credit := v_credit_limit + v_current_balance;
    
    -- Check if this purchase would exceed their credit limit
    IF v_new_balance < -v_credit_limit THEN
        RETURN json_build_object(
            'success', false,
            'error', 'Purchase would exceed credit limit. Available credit: ' || v_available_credit
        );
    END IF;
...
-- Function to process account payment (add money to account)
...
BEGIN
    ...
    -- Update customer balance (add the payment amount)
    UPDATE customers 
    SET 
        account_balance = v_new_balance,
        updated_at = NOW()
...
-- Function to add money to customer account (deposit)
CREATE OR REPLACE FUNCTION add_account_deposit(
    p_customer_id UUID,
    p_amount NUMERIC,
...
BEGIN
    ...
    -- Update customer balance (add the deposit amount)
    UPDATE customers 
    SET 
        account_balance = v_new_balance,
        updated_at = NOW()
    ...
    RETURN json_build_object(
        ...
        'old_balance', v_current_balance,
        'new_balance', v_new_balance,
        'amount_deposited', p_amount,
        'message', 'Account deposit processed successfully'
    );
END;
$$ LANGUAGE plpgsql;
...
INSERT INTO customers (customer_number, first_name, last_name, email, phone, account_balance, credit_limit) VALUES
('CUST001', 'John', 'Doe', 'john.doe@example.com', '+1234567890', 200.00, 1000.00), -- Has R200 in account, can go R1000 into debt
('CUST002', 'Jane', 'Smith', 'jane.smith@example.com', '+1234567891', 0.00, 1500.00), -- No money, can go R1500 into debt
('CUST003', 'Bob', 'Johnson', 'bob.johnson@example.com', '+1234567892', 500.00, 2000.00); -- Has R500, can go R2000 into debt
...
-- Success message
SELECT 'Corrected credit system setup completed successfully!' as status;
