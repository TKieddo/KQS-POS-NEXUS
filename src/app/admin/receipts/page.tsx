...
import { autoSetupReceiptTemplates, getTemplateStatus } from "@/lib/auto-setup-templates"
...
const DynamicReceiptPreview: React.FC<{ template: ReceiptTemplate; className?: string }> = ({ 
  ...
  const getTemplateComponent = () => {
    ...
    if (name.includes('final laybye payment')) {
      return <LaybyeFinalPaymentReceiptPreview className={className} />
    }
    if (name.includes('laybye') && name.includes('payment')) {
...
const ReceiptsPage = () => {
  ...
  useEffect(() => {
    ...
    // Auto-setup templates if needed
    const setupTemplates = async () => {
      try {
        const status = await getTemplateStatus(selectedBranch.id)
        if (!status.ready) {
          console.log('Auto-setting up missing templates...')
                  const result = await autoSetupReceiptTemplates(selectedBranch.id)
        if (result.length === 0) {
          console.log('Auto-setup completed successfully')
          // Reload templates after auto-setup
          loadTemplates()
        } else {
          console.log('Auto-setup completed with errors:', result)
        }
        }
      } catch (error) {
  ...
  const saveTemplate = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.template) {
        ...
        successMessage.textContent = '✅ Template saved successfully to database!'
        ...
        errorMessage.textContent = `❌ Error saving template: ${result.error}`
      ...
      const errorMessage = document.createElement('div')
      errorMessage.className = 'fixed top-4 right-4 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg z-50'
      errorMessage.textContent = '❌ An unexpected error occurred while saving the template.'
      document.body.appendChild(errorMessage)
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      setAutoSaving(true)
      saveTemplate().finally(() => {
        setAutoSaving(false)
      })
    }, 3000)
    
    return () => clearTimeout(timeoutId)
  }, [hasUnsavedChanges, template, saveTemplate])
  ...
  const handleEdit = useCallback((templateId: string) => {
    ...
    if (selectedTemplate) {
      ...
      // Scroll to the template editor section
      setTimeout(() => {
    ...
    <main className="min-h-screen bg-[hsl(var(--background))] flex flex-col items-center py-10 px-4">
      <div className="w-full max-w-7xl">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          {/* Template Editor */}
          {showEditor && (
            <div id="template-editor-section" className="lg:w-2/5">
              <TemplateEditor
                template={template}
          ...
          <div className={`flex-1 flex flex-col items-center ${showEditor ? 'lg:w-3/5' : 'w-full'}`}>
