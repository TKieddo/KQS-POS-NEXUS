...
import { Calendar, Download, RotateCcw, BarChart2, X } from 'lucide-react'
...
// Mock data
const stats = [
  { label: 'Total Refunds', value: 2500, icon: RotateCcw },
  { label: 'Refund Rate', value: '5.2%', icon: BarChart2 },
  { label: 'Avg Refund', value: 180, icon: Calendar },
]
const refundTrend = [
  { date: '2024-06-01', value: 200 },
  { date: '2024-06-02', value: 350 },
  { date: '2024-06-03', value: 400 },
  { date: '2024-06-04', value: 300 },
  { date: '2024-06-05', value: 500 },
  { date: '2024-06-06', value: 450 },
  { date: '2024-06-07', value: 300 },
]
const refundTransactions = [
  { id: 1, date: '2024-06-01', amount: 120, reason: 'Damaged item', cashier: 'Jane Doe', method: 'Card' },
  { id: 2, date: '2024-06-02', amount: 200, reason: 'Customer return', cashier: 'John Smith', method: 'Cash' },
  { id: 3, date: '2024-06-03', amount: 180, reason: 'Wrong item', cashier: 'Alice Brown', method: 'Mpesa' },
]

const chartData = {
  labels: refundTrend.map((d) => d.date),
  datasets: [
    {
      label: 'Refunds',
      data: refundTrend.map((d) => d.value),
...
function RefundDetailsModal({ refund, open, onClose }: { refund: any, open: boolean, onClose: () => void }) {
  if (!open) return null
  return (
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md relative animate-fade-in">
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="p-3 rounded-xl bg-black shadow-sm">
            <RotateCcw className="h-8 w-8 text-[#E5FF29]" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-black mb-1">Refund Details</h2>
            <div className="text-xs text-muted-foreground">Date: {refund.date}</div>
            <div className="text-xs text-muted-foreground">Cashier: {refund.cashier}</div>
            <div className="text-xs text-muted-foreground">Method: {refund.method}</div>
          </div>
        </div>
        <div className="mb-2 text-sm text-black">Reason: {refund.reason}</div>
        <div className="flex gap-4 mt-4">
          <div className="flex flex-col items-center">
            <span className="text-xs text-muted-foreground">Amount</span>
            <span className="font-bold text-lg">R{refund.amount.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
...
const RefundsReportPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-6 text-black">Refunds</h1>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <p className="text-muted-foreground">Monitor refund frequency and amounts.</p>
      ...
      {loading ? (
        ...
        <>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {refundTransactions.length === 0 ? (
                  <tr><td colSpan={6} className="text-center py-8 text-muted-foreground">No refunds found.</td></tr>
                ) : (
                  refundTransactions.map((r) => (
                    <tr key={r.id} className="border-b border-border hover:bg-[#E5FF29]/10 transition">
                      <td className="py-2 px-2">{r.date}</td>
                      <td className="py-2 px-2 text-right font-bold">R{r.amount.toLocaleString()}</td>
                      <td className="py-2 px-2">{r.reason}</td>
                      <td className="py-2 px-2">{r.cashier}</td>
                      <td className="py-2 px-2">{r.method}</td>
                      <td className="py-2 px-2 text-right">
                        <button className="flex items-center gap-1 font-semibold hover:underline" onClick={() => setSelectedRefund(r)}>
                          <span className="p-1 rounded-full bg-black shadow-sm"><RotateCcw className="h-4 w-4 text-[#E5FF29]" /></span>
                          Details
                        </button>
                      </td>
                    </tr>
          ...
          <RefundDetailsModal refund={selectedRefund} open={!!selectedRefund} onClose={() => setSelectedRefund(null)} />
        </>
      )}
    </div>
  )
