...
interface QuotationSlipReceiptPreviewProps {
  className?: string
}

const QuotationSlipReceiptPreview: React.FC<QuotationSlipReceiptPreviewProps> = ({ className }) => {
  // Sample data for quotation slip
  const sampleData = {
    business_name: 'KQS',
    business_address: 'Maseru, Husteds opposite Queen II',
    business_phone: '2700 7795',
    business_website: 'kqs-boutique.com',
    quote_number: 'KQS-QT-2024-00055',
    date: '27-Dec-24',
    time: '10:00 AM',
    cashier_name: 'Hape',
    customer_name: 'NTEBALENG TAELO',
    company_name: 'Taelo Holdings',
    expiry_date: '03-Jan-25',
    items: [
      { name: 'Nike Air Max 2025', quantity: 5, price: 2200.00, total: 11000.00, category: 'Shoes' },
      { name: 'Custom Leather Jacket', quantity: 3, price: 3500.00, total: 10500.00, category: 'Clothing' }
    ],
    subtotal: 21500.00,
    discount: 1500.00,
    total: 20000.00
  ...
  return (
    <PremiumCard className={`p-6 ${className}`}>
      <div className="text-center mb-6">
        <h3 className="text-xl font-semibold text-gray-900 mb-2">KQS Quotation Slip</h3>
        <p className="text-sm text-gray-600">Quotation for bulk/group/company purchases</p>
      </div>
      ...
      <div className="bg-white border-2 border-gray-200 rounded-lg p-4 max-w-sm mx-auto font-mono text-xs shadow-lg relative">
        ...
        <div className="mb-3 space-y-1">
          <div className="text-gray-700">Quote #: {sampleData.quote_number}</div>
          ...
          <div className="text-gray-700">Company: {sampleData.company_name}</div>
          <div className="text-gray-700">Valid Until: {sampleData.expiry_date}</div>
        </div>
        ...
        {/* Quotation Policy */}
        <div className="mb-3">
          <div className="text-center font-bold text-gray-900 mb-1">Quotation Policy</div>
          <div className="text-gray-700 text-xs leading-tight bg-gray-50 p-2 rounded-sm">
            Quotation is valid until the expiry date. Prices and availability may change after expiry.<br />
            <span className="italic text-gray-600">
              Quote ena e nakong n ho fihlela letsatsi la ho felloa ke nako le fanoeng. price kapa lenane li ka fetoha neng kapa neng kamora ho etsa ea quotation ena.
            </span>
          </div>
        </div>
...
export default QuotationSlipReceiptPreview 