-- Corrected account payment functions
-- These functions properly handle balance deduction and credit purchases

-- Function to process account payments (subtract from customer balance)
...
-- Function to update customer balance (subtract amount)
...
-- Function to handle credit purchases (when customer buys on credit)
...
BEGIN
    ...
    IF v_new_balance > v_credit_limit THEN
        RETURN json_build_object(
            'success', false,
            'error', 'Purchase would exceed credit limit'
        );
    END IF;
    ...
    RETURN json_build_object(
        'success', true,
        'new_balance', v_new_balance,
        'available_credit', v_credit_limit - v_new_balance,
